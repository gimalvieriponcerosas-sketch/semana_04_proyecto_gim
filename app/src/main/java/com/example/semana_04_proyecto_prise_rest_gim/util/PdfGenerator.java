...
public class PdfGenerator {
    ...
    public static String generateActa(Context context, Fiscalizacion fiscalizacion,
    ...
    private static void addHeader(Document document, Context context, String expediente) {
        ...
        int resId = context.getResources().getIdentifier("img", "drawable",
        ...
        if (resId != 0) {
            ...
            leftCell.add(new Paragraph("OSINERGMIN").setBold().setFontSize(14).setFontColor
                    (ColorConstants.BLUE));
        }
        ...
        expBox.addCell(new Cell().add(new Paragraph("EXPEDIENTE Nro.").setFontSize(9)).
    ...
    private static void addTitle(Document document) {
        document.add(new Paragraph("Acta de Fiscalización del Cumplimiento del " +
                "Procedimiento de Entrega de Información de Precios de Combustibles " +
                "Derivados de Hidrocarburos PRICE")
    ...
    private static void addAgentInfo(Document document, Establecimiento est, Fiscalizacion fis) {
        ...
        table.addCell(new Cell(1, 3).add(new Paragraph("AGENTE FISCALIZADO: " +
                "" + est.getNombre().toUpperCase())));
        table.addCell(new Cell().add(new Paragraph("CÓDIGO OSINERGMIN: " + est.getCodigoOsinergmin())));
        table.addCell(new Cell(1, 2).add(new Paragraph("REGISTRO HIDROCARBUROS N: "
                + est.getRegistroHidrocarburos())));
        ...
        table.addCell(new Cell(1, 3).add(new Paragraph("DIRECCIÓN: "
                + est.getDireccion().toUpperCase())));
        ...
        table.addCell(new Cell().add(new Paragraph("DEPARTAMENTO: HUANUCO")
        ...
        table.addCell(new Cell(1, 3).add(new Paragraph("FISCALIZADOR DE OSINERGMIN: "
                + fis.getFiscalizadorResponsable().toUpperCase()).setBold()));
        document.add(table);
    }
    private static void addPreciosTable(Document document, List<Precio> precios) {
        document.add(new Paragraph("I. INFORMACIÓN RECABADA:").setBold().setFontSize(8)
                .setMarginTop(5).setUnderline());

        // Tabla con estructura compleja (Anidada)
        Table mainTable = new Table(UnitValue.createPercentArray(new float[]
                {2, 1, 1, 1, 1, 1, 0.5f, 1, 0.3f, 0.3f, 0.3f, 0.3f, 0.3f}));
        ...
        mainTable.addHeaderCell(new Cell(2, 1).add(new Paragraph("Otros"))
                .setTextAlignment(TextAlignment.CENTER));
        mainTable.addHeaderCell(new Cell(2, 1).add(new Paragraph("Marca"))
                .setTextAlignment(TextAlignment.CENTER));
        mainTable.addHeaderCell(new Cell(1, 5).add(new Paragraph
                ("GLP Envasado en Cilindros - Kilogramos:")).setTextAlignment(TextAlignment.CENTER));
        ...
        mainTable.addHeaderCell(new Cell().add(new Paragraph("10"))
                .setTextAlignment(TextAlignment.CENTER));
        mainTable.addHeaderCell(new Cell().add(new Paragraph("15"))
                .setTextAlignment(TextAlignment.CENTER));
        mainTable.addHeaderCell(new Cell().add(new Paragraph("45"))
                .setTextAlignment(TextAlignment.CENTER));
        // Filas de datos
        String[] labels = {"Precio registrado en PRICE", "Precio publicado en establec.",
                "Precio consignado en surtidor", "Precio con descuento PRICE"};
        for (int row = 0; row < 4; row++) {
            mainTable.addCell(new Cell().add(new Paragraph(labels[row])));
            // Llenamos las columnas con datos reales de la lista 'precios'
            for (int col = 0; col < 12; col++) {
                String value = "";
                if (col < precios.size()) {
                    Precio p = precios.get(col);
                    if (row == 0) value = String.valueOf(p.getPrecioPrice());
                    if (row == 1) value = String.valueOf(p.getPrecioPublicado());
                    if (row == 2) value = String.valueOf(p.getPrecioSurtidor());
                    if (row == 3) value = String.valueOf(p.getPrecioDescuento());
                }
                mainTable.addCell(new Cell().add(new Paragraph(value)).setTextAlignment(TextAlignment.CENTER));
            }
        }
    ...
    private static void addVerificaciones(Document document, List<Verificacion> verifs) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            table.addCell(new Cell().add(new Paragraph("Si")).setBackgroundColor(res ?
                    ColorConstants.YELLOW : ColorConstants.WHITE).setTextAlignment(TextAlignment.CENTER));
            table.addCell(new Cell().add(new Paragraph("No")).setBackgroundColor(!res ?
                    ColorConstants.YELLOW : ColorConstants.WHITE).setTextAlignment(TextAlignment.CENTER));
        }
        document.add(table);
        document.add(new Paragraph("* Para el caso de Productores... ** Aplica a Productores... *** " +
                "Aplica a Plantas Envasadoras...").setFontSize(5));
    }
    private static void addHechosVerificados(Document document,
                                             List<HechoVerificado> hechos, int start, int end) {
        if (start == 1) document.add(new Paragraph("II. HECHOS VERIFICADOS:").setBold().setFontSize(8)
                .setMarginTop(5).setUnderline().setTextAlignment(TextAlignment.CENTER));
        ...
        table.addHeaderCell(new Cell().add(new Paragraph("N°")).setBackgroundColor(ColorConstants
                .LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));
        table.addHeaderCell(new Cell().add(new Paragraph("INCUMPLIMIENTO"))
                .setBackgroundColor(ColorConstants.LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));
        table.addHeaderCell(new Cell().add(new Paragraph("HECHOS VERIFICADOS"))
                .setBackgroundColor(ColorConstants.LIGHT_GRAY).setTextAlignment(TextAlignment.CENTER));
        for (int i = start; i <= end; i++) {
            table.addCell(new Cell().add(new Paragraph(String.valueOf(i))).setTextAlignment(TextAlignment.
                    CENTER));
        ...
        document.add(table);
        if (start == 1) {
            document.add(new Paragraph("ÍTEMS ADICIONALES PARA PLANTAS ENVASADORAS, LOCALES DE VENTA, " +
                    "GRIFOS...").setBold().setFontSize(7)
                    .setTextAlignment(TextAlignment.CENTER).setBorder(new SolidBorder(0.5f)));
        }
    }
    private static void addOtrosSection(Document document) {
        ...
        table.addCell(new Cell().add(new Paragraph("Documentación recabada en la fiscalización:" +
                "\n\n")));
        table.addCell(new Cell().add(new Paragraph("Manifestaciones u observaciones " +
                "del Agente Fiscalizado:\n\n")));
    ...
    private static void addFirmas(Document document, List<Firma> firmas, Fiscalizacion fis) {
        Table table = new Table(2).setWidth(UnitValue.createPercentValue(100)).setMarginTop(30)
                .setFontSize(7);
        String nombreRecibe = "";
        String dniRecibe = "";
        ...
        rCell.add(new Paragraph("Firma de quien recibe").setBold());rCell.add(new Paragraph("DNI: " + dniRecibe));
    ...
    private static void addBaseLegal(Document document) {
        document.add(new Paragraph("Base Legal: Texto Único Ordenado de la Ley N° 27444," +
                " Ley del Procedimiento Administrativo General, aprobado por Decreto Supremo Nº 004-2019-JUS; Ley de Creación de Osinergmin...")
    ...
    private static void addFooter(Document document, int page, String expediente) {
        ...
        footer.addCell(new Cell().add(new Paragraph("PARA TRÁMITES POSTERIORES REFERENTES A ESTA" +
                " FISCALIZACIÓN, SEÑALAR EL NÚMERO DE EXPEDIENTE: " + expediente).setFontSize(7))
                .setPadding(5));document.add(footer);
        document.add(new Paragraph("Página " + page + " de 2").setTextAlignment(TextAlignment.CENTER)
